...
impl Universe {
    ...
    // In order to calculate the next state of a cell, we need 
    // to get a count of how many of its neighbors are alive.
    fn live_neighbor_count(&self, row: u32, column: u32) -> u8 {
        ...
        for delta_row in [self.height - 1, 0, 1].iter().cloned() {
            for delta_col in [self.width - 1, 0, 1].iter().cloned() {
                ...
                // uses deltas and modulo to avoid special casing the edges of the universe
